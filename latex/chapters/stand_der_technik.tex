...
\begin{figure}[htb]
    ...
    \caption{Variation der Dichte und des Brechungsindexs bei unterschiedlichem Drucken~\cite{gohar_1988}}
...
Flüssigkeiten, deren Viskosität mit steigenden Schubspannungen zunehmen, nennt man dilatant.
Solches Verhalten zeigt häufig Suspensionen an, die sich nicht als Schmierstoff eigen.
...
Sie ist entgegen der Bewegungsrichtung gerichtet und wandelt die mechanische Energie in Wärme um.
Man unterscheidet zwischen innerer und äußerer Reibung.
Äußere Reibung tritt auf, wo die Flächen der unterschiedlichen Körper sich berühren.
...
Da die Gasreibung für diese Arbeit irrelevant ist, wird kurz auf die einzelnen Reibungszustände ohne Gasreibung eingegangen.
...
Dabei besteht bei hohen Flächenpressungen die Gefahr, dass die beiden Körper aneinander haften, verscheißen und es zum Festfressen kommen kann.
...
Diese Schicht kann zum Beispiel durch Oxidation, Adsorption oder chemische Reaktionen entstanden sein und vermindert die Reibung im Vergleich zur Festkörperreibung schon eheblich.
...
Bei Flüssigkeitsreibung handelt es sich um eine vollständige Trennung der Laufflächen.
...
Die hydrodynamische Schmierung tritt im Allgemeinen nur bei konformen Kontaktpaarungen auf.
...
Die Kontaktpaarungen bei dieser Schmierung sind im Allgemeinen kontraform.
...
Im Gegenteil zu den konformen Kontakten, wo die Pressungen in der Größenordnung von \SI{10}{\mega\pascal} auftreten, können nichtkonforme Kontakte, die sich zwischen den Laufflächen befinden, die Druckspannungen von \SI{0.5}{\giga\pascal} und höher aufweisen.
Durch die enorme, konzentrierte Belastung werden die Flächen an dem Kontaktpunkt elastisch verformt und vergrößert.
Abbildung~\ref{fig:druckverteilung_im_ehd_kontakt} zeigt die Druck- und Temperaturverteilung und wie die Körper verformen in einem EHD-Kontakt an.

% ----------------------------------------
% Fig: Druckverteilung im EHD-Kontakt
% ----------------------------------------
\begin{figure}[htb]
    \centering
    \includegraphics[width=0.6\textwidth]{./images/druckverteilung_im_ehd_kontakt.png}
    \caption{Druck- und Temperaturverteilung in einem konzentrierten Konakt~\cite{tum_tribo}}
    \label{fig:druckverteilung_im_ehd_kontakt}
\end{figure}

\subsection{Kontakt von beliebig gekrümmten Elementen}
\label{sub:kontakt_von_beliebig_gekruemmten_elementen}

Bei der Betrachtung des konzentrierten Kontaktes werden alle Oberflächen als ideal glatt angenommen.
...
Zwischen den beiden Ebenen, die $r_{11}$ und $r_{21}$ enthalten, bildet sich der Winkel $\varphi$.
...
Im Kontaktpunkt bilden die Kontaktfläche eine Ellipse mit den Halbachsen $a$ und $b$.
...
Das reduzierte Elastizitätsmodul $E'$ beschreibt die elastischen Eigenschaften der beiden Elemente und wird definiert als
% ----------------------------------------
...
\end{equation}
%
wobei $\nu_x$ die Querkontraktionszahl der Kontaktkörper und $E_x$ das Elastizitätsmodul der Kontaktkörpern ist.
...
% ----------------------------------------
\input{./tables/einfluesse_auf_schmierfilmdicke.tex}
