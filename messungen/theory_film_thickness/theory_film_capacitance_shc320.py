...
oil_dichte = np.array([846.85, 834.98, 823.10])
# kinematische Viskosität bei 40, 60 und 80 C [m^2/s]
oil_ki_viskositaet = np.array([329.14, 133.72, 66.44]) * 1e-6
...
for geschw in geschwindigkeiten:
    for i, last in enumerate(lasten):
        for j, temp in enumerate(temperaturen):
            ...
            temp_data = ["SHC320", temp, last, pressung, geschw, h_0, cap]
            data.loc[len(data)] = temp_data

# export the data in csv file
# data.to_csv("data_shc320.csv", sep='\t', index=False)
...
ax1.plot(data_80C_40N["Speed"], data_80C_40N["Film"] * 1e9, "b-")
ax1.set_xlabel("Speed [m/s]")

# make the y-axis label, ticks and tick labels match the some color
ax1.set_ylabel("Film [nm]", color='b')
ax1.tick_params(axis='y', colors='b')

ax2 = ax1.twinx()
ax2.plot(data_80C_40N["Speed"], data_80C_40N["Cap"] * 1e12, "r-")
ax2.set_ylabel("Cap [pF]", color='r')
ax2.tick_params(axis='y', colors='r')

fig.tight_layout()
# fig.savefig("film_cap_speed_80C_20N_SHC320.pdf",
...
film_ax = np.linspace(min_film, max_film, group_length) * 1e9  # scale to nm
